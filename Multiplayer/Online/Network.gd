...
func create_server() -> void:
	...
	Global.instance_node(load("res://Multiplayer/Online/Server_Advertiser.tscn"), get_tree().current_scene)
...
func _server_disconnected() -> void:
	...
	if Global.ui != null:
		...
		var prompt = Global.instance_node(load("res://Multiplayer/Online/Simple_prompt.tscn"), Global.ui)
		prompt.set_text("Disconnected from server")

func _client_connection_timeout():
	if client_connected_to_server == false:
		print("Client has been time out")
		
		reset_network_connection()
		
		var connection_timeout_prompt = Global.instance_node(load("res://Multiplayer/Online/Simple_prompt.tscn"), get_tree().current_scene)
